...
public class Contest extends BaseEntity{
    ...
    private void validateQuestionList(List<Question> qList, Level contestLevel) throws InvalidContestException {
        if(qList==null || qList.isEmpty()){
            throw new InvalidContestException("Contest must have at least one question.");
        }
        boolean allQuestionsMatch=qList.stream().allMatch(q->q.getLevel()==contestLevel);
        if(!allQuestionsMatch){
            throw new InvalidContestException("All questions must have the same level as the contest.");
        }
    }
