...
public class CreateContestCommand implements ICommand{
    ...
    public void execute(List<String> tokens) throws UserNotFoundException,QuestionNotFoundException {
        //List.of("CREATE-CONTEST","name","LOW","creator","2")
        String name=tokens.get(1);
        String level=tokens.get(2);
        String user=tokens.get(3);
        // Integer numOfQ=tokens.size()==5?Integer.valueOf(tokens.get(4)):null;
        Contest contest;
        try {
         contest=contestService.create(name,  Level.valueOf(level), user, tokens.size()==5?Integer.valueOf(tokens.get(4)):null);
