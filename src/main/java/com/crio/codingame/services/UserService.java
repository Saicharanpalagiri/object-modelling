...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
        ...
        if(scoreOrder==ScoreOrder.ASC){
            return users.stream().sorted(Comparator.comparingInt(User::getScore))
            .collect(Collectors.toList());
        }else{
            return users.stream().sorted(Comparator.comparingInt(User::getScore).reversed())
            .collect(Collectors.toList());
        }
    }
    ...
    if (!user.checkIfContestExists(contest)) {
        throw new InvalidOperationException("Cannot Withdraw Contest. User is not registered for the contest:" + contestId);
    }
